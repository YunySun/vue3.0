...
ScrollReader.prototype = {
    initEvents: function() {
        ...
        $nextTrigger.on("click", function() {
            layer_login=layer.open({type: 2,content: '鍔犺浇涓�'});
            self.enabled = false;
            self.onScrollNext(function() {
...
(function ($) {
    ...
    Aside.prototype.show = function(hash) {
        ...
        el.overlay.attr("title", "鐐瑰嚮鍏抽棴娴眰");
        elTrigger.attr("title", "娴眰宸叉樉绀�");
...
var Range = function(el, options) {
    ...
    if (params.shadow == true) {
        shadow = $("<a></a>").addClass(rangePrefix + "shadow").attr("title", "涔嬪墠浣嶇疆");
    }
    el.before(container);
